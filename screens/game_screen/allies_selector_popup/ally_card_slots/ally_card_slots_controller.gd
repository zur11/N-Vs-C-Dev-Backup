class_name AllyCardSlotsController extends InputController

var default_containing_scene : Node
...
func _on_input_event_key_pressed(event:InputEvent):
	...
	if containing_scene == null:
		containing_scene = default_containing_scene
	
	if event.pressed:
		var focused_object_index : int = focusable_objects.find(focused_object)
		var new_focused_object : Control 
		if not focused_object_index >= 0 and focused_object != null:
			new_focused_object = focused_object.assigned_ally_card
			focused_object_index = focusable_objects.find(new_focused_object)
		...
		if event.is_action("display_pause_menu"): # START KEY in Nintendo Controls
			...
			if event is InputEventScreenTouch:
				_disable_keys_input_control()
				return
			if focused_object is ChoosableAllyCard:
				focused_object.pressed.emit()
				containing_scene.on_ally_card_unselected()
			else:
				if focused_object != null:
					if focused_object.assigned_ally_card != null:
						focused_object.assigned_ally_card.pressed.emit()
					containing_scene.on_ally_card_unselected()
				else:
					containing_scene.on_ally_card_unselected()
					return
