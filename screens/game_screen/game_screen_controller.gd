...
func _on_input_event_key_pressed(event:InputEvent):
	...
	if event.pressed:
		...
		if event.is_action("display_pause_menu"):
			containing_scene.on_start_key_input_received(containing_scene)
		...
		elif event.is_action("select_last_coin"):
			containing_scene.on_coins_selection_key1_input_received()
		elif event.is_action("select_first_coin"):
